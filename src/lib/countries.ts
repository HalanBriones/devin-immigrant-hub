// Static ISO 3166-1 alpha-2 list with English display names, generated once from
// Intl.DisplayNames. Kept static so server and client render identical option lists —
// ICU data differs between Node and browsers, which otherwise causes hydration mismatches.
...
const NAMES_BY_CODE = new Map(COUNTRIES.map((country) => [country.code, country.name]));

export function countryName(code: string | null | undefined): string | null {
  if (!code) return null;
  return NAMES_BY_CODE.get(code) ?? code;
}
